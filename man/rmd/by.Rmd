...
When used with `.by`, `summarise()`, `morph()`, and `slice()` all maintain the ordering of the existing data.
...
If you need sorted group keys, we recommend that you explicitly use [arrange()] either before or after the call to `summarise()`, `morph()`, or `slice()`.
