...
sql_join.bigquery <- function(con, x, y, type = "inner", by = NULL, ...) {
  join <- switch(type,
                 left = sql("LEFT JOIN EACH"),
                 inner = sql("INNER JOIN EACH"),
                 right = sql("RIGHT JOIN EACH"),
                 full = sql("FULL JOIN EACH"),
  ...
  if (type == "cross") {
    cond <- build_sql("", con = con)

    non_join_vars <- sel_vars
    
    sql_var_list <- sql_vector( c(sql_escape_ident(con, non_join_vars)),
