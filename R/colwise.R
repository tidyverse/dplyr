...
#' @param .cols A list of columns generated by \code{\link{columns}()},
...
is_columns <- function(cols) inherits(cols, "col_list")

select_colwise_names <- function(tbl, cols) {
  vars <- tbl_vars(tbl)

  if (is.character(cols) || is_columns(cols)) {
    selected <- cols
...
#' Deprecated _each() variants
#'
#' \code{mutate_each()} and \code{summarise_each()} are deprecated in
#' favour of a more featureful family of functions:
