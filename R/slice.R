...
slice_min.data.frame <- function(.data, order_by, ..., n, prop, with_ties = TRUE) {
  ...
  slice_fix_error(call = call("slice_min"),
    slice(.data, {
      n <- dplyr::n()
      x <- slice_vec_assert({{ order_by }}, size = n, arg = "order_by")
      idx(x, n)
    })
  )
}
...
slice_max.data.frame <- function(.data, order_by, ..., n, prop, with_ties = TRUE) {
  ...
  slice_fix_error(call = call("slice_max"),
    slice(.data, {
      n <- dplyr::n()
      x <- slice_vec_assert({{ order_by }}, size = n, arg = "order_by")
      idx(x, n)
...
slice_sample.data.frame <- function(.data, ..., n, prop, weight_by = NULL, replace = FALSE) {
  ...
  slice_fix_error(call = call("slice_sample"),
    slice(.data, {
      n <- dplyr::n()
      weight_by <- {{ weight_by }}
      if (!is.null(weight_by)) {
        weight_by <- slice_vec_assert(weight_by, size = n, arg = "weight_by")
      }
...
slice_vec_assert <- function(...) {
  wrap_error(vec_assert(...), class = "slice_assert_error")
}

# This is meant to simplify errors that are generated by slice()
# when slice is called from slice_sample(), ... because the user
# does not want to see the quosure passed to slice() as this is
# made up internally.
slice_fix_error <- function(expr, call) {
  withCallingHandlers(
    expr,
    error = function(cnd) {
      if (inherits(cnd$parent, "slice_assert_error")) {
        # this was thrown by a slice_vec_assert(): the error can be simplified
        msg <- c(
...
slice_rows <- function(.data, ..., caller_env) {
  ...
  for (group in seq_along(rows)) {
    ...
    if (is.logical(res) && all(is.na(res))) {
      ...
      abort(msg, call = call("slice"), class = "dplyr_error")
    }
  ...
  withCallingHandlers(
    ...
    error = function(cnd) {
      ...
      abort(
        ...
        # might be needed by slice_fix_error()
        cur_group_label = cnd_bullet_cur_group_label()
      )
    }
  )
