...
#' of [morph()] (which can produce multiple rows and multiple columns),
...
#' @export
#' @examples
#' # do() with unnamed arguments becomes morph() or summarise()
#' # . becomes pick()
#' by_cyl <- mtcars %>% group_by(cyl)
#' by_cyl %>% do(head(., 2))
#' # ->
#' by_cyl %>% morph(head(pick(everything()), 2))
