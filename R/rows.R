...
rows_insert.data.frame <- function(x, y, by = NULL, ..., copy = FALSE, in_place = FALSE) {
  ...
  bad <- which(!is.na(idx))
  if (has_length(bad)) {
    abort(class = "dplyr:::error_rows_insert_duplicate",
...
rows_update.data.frame <- function(x, y, by = NULL, ..., copy = FALSE, in_place = FALSE) {
  ...
  bad <- which(is.na(idx))
  if (has_length(bad)) {
    abort(class = "dplyr:::error_rows_update_missing",
...
rows_patch.data.frame <- function(x, y, by = NULL, ..., copy = FALSE, in_place = FALSE) {
  ...
  bad <- which(is.na(idx))
  if (has_length(bad)) {
    abort(class = "dplyr:::error_rows_patch_missing",
...
rows_delete.data.frame <- function(x, y, by = NULL, ..., copy = FALSE, in_place = FALSE) {
  ...
  bad <- which(is.na(idx))
  if (has_length(bad)) {
    abort(class = "dplyr:::error_rows_delete_missing",
