...
translate_env.src_greenplum <- function(x) {
  ...
  sql_variant(
    ...
    sql_translator(.parent=base_win,
                   mean  = function(x) {
                     over(build_sql(sql("avg"), list(x)), partition_group(), frame = NULL)
                   },
                   sum   = function(x) {
                     over(build_sql(sql("sum"), list(x)), partition_group(), frame = NULL)
                   },
                   min   = function(x) {
                     over(build_sql(sql("min"), list(x)), partition_group(), frame = NULL)
                   },
                   max   = function(x) {
                     over(build_sql(sql("max"), list(x)), partition_group(), frame = NULL)
                   },
                   n = function() {
