...
#' @param coalesce for `right_join` and `full_join`, controls which values are
#'   returned in `by` columns for rows in `y` with no match in `x`.
#'
#'   If `FALSE`, each `by` column in the result is retuned from `x`. `by`
#'   columns in the result will contain `NA` values for rows from `y` that have
#'   no match in `x`.
#'
#'   If `TRUE`, each `by` column in the result is generated by coalescing the
#'   corresponding `by` columns in `x` and `y`. `by` columns in the result will
#'   contain values from `y` for rows from `y` that have no match in `x`. Set to
#'   `TRUE` to match the behavior of joins on local data frames.
#' @examples
#' \dontrun{
