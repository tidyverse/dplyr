...
*   If you have the name in an env-variable, you can use glue syntax to interpolate 
    in:
...
*   If you the name should be derived from a data-variable in an 
    argument, you can use embracing syntax:
...
If you want to use the names of variables in the output, you can use embrace the variable name on the left-hand side of `:=`:
...
If you want to give the user the ability to control the generated names, add an additional argument that uses `rlang::caller_arg()` to get the name of the argument to the function:

```{r}
quantile_df <- function(x, probs = c(0.25, 0.5, 0.75), prefix = rlang::caller_arg(x)) {
  tibble(
...
Note that I've switched from embracing (`{{}}`) to glue-style interpolation (`{}`) since prefix is now a regular string.
