...
```{r}
quantile_df <- function(x, probs = c(0.25, 0.5, 0.75), prefix = englue("{{x}}")) {
  tibble(
...
`englue()` is the tool that powers embracing in variable names, so here we're just using it explicitly so that the user can override if needed. Also note that inside the tibble I've switched from embracing (`{{}}`) to glue-style interpolation (`{}`) since `prefix` is now a regular string.
