...
problem, which we call the tidy evaluation framework.

There are two importants aspects to this framework: quoting and
unquoting. In addition, the notion of pronouns is also helpful. We go
over these three aspects in this vignette.
...
A different approach to solve scoping problems is to use
pronouns. Most tidyverse functions provide two pronouns that you can
refer to without unquoting: `.data` refers to the data structure that
is put in scope, and `.env` refers to the calling context. These two
symbols are always defined:

```{r}
var <- 10
summarise(mtcars, mean = mean(.data$cyl / .env$var))
```
