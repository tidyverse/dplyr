...
*   Most dplyr arguments are not __referentially transparent__. That means
    ...
    *Jenny: I still think `var <- "my_var"; filter(df, var == 1)` is a better example. As in, more reasonable people have false hope that it will work. I think it's too clear that the above won't work because it's obvious that `var <- my_var` can't possibly do the right thing. But it doesn't take too much magical thinking to kindle hope that my example works. One last concern: do we worry some people will get distracted by re: concern about masking `var()`?*
...
*Jenny: Feels like "vary over" can be improved upon. Try to propose better wording for this concept: plumbing up stuff you compute in your function, probably using your inputs, to arguments of dplyr functions.*
...
*Jenny: Sidebar: could that error message be improved? "No object `a` exists in `.data`."*
...
Instead we need to capture, or __quote__, the specification of the group variable as an __expression__. There are two ways to do this in base R: `quote()` and the formula, `~`.

*Jenny: "the formula, `~`" feels squishy. Either needs to talk about creating a formula or say something about the tilde operator. Or both.*
...
*Jenny: could some brief explanation be provided? I feel like anyone reading this is game to learn more about what's really going on.*
...
*Jenny: I'm at pedantry level 10 now. Reader might wonder what the relationship is between an expression and a formula. Because the words have recently been used as if they're interchangeable.*
...
*Jenny: I would describe this as `quo()` being too literal? Is there a better way to say that?*
...
*Jenny: I'm still left a bit unsatisfied re: the need to switch from `quo()` to `enquo()`. Feels like it needs more words.*
