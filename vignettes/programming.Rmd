...
If your function returns multiple _rows_ per group, then you'll need to switch from `summarise()` to `morph()`. `summarise()` is restricted to returning 1 row summaries per group, but `morph()` lifts this restriction:

```{r}
df %>%
  group_by(grp) %>%
  morph(across(x:y, quantile_df, .unpack = TRUE))
```
