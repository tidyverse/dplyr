...
To determine whether a function argument uses data masking or tidy selection, look at the documentation: in the arguments list you'll see `<tidy-eval>` or `<tidy-select>`.
...
### One user-supplied expression

If you want the user the to supply an expression that's passed on to an argument that uses data masking or tidy select, embrace the argument:
...
### Multiple user-supplied expressions
...
Index into the special `.data` pronoun:
...
This same technique works with the base R `apply()` family and the purrr `map()` family:

```{r, results = FALSE}
mtcars %>% 
  names() %>% 
  purrr::map(~ mtcars %>% count(.data[[.x]]))
```
