...
* How individual dplyr verbs change their behaviour when applied to grouped 
  data frame.
...
`cur_data()` returns the current group, excluding grouping variables. It's useful to feed to functions that take a whole data frame. For example, the following code fits a linear model of `mass ~ height` to each species that has more than one member:
