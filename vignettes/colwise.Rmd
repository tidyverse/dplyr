...
* `if_any()` to keep the rows for which the predicate is true for *at least one* selected
  column: 
...
* `if_all()` to keep the rows where the predicate is true for *all* selected columns:
