...
(If you're trying to compute `mean(a, b, c, d)` for each row, instead see `vignette("colwise")`)

This vignette will introduce you to the `across()` function, which lets you write the more succinct:
...
We'll start by discussing the basic usage of `across()`, particularly as it applies to `summarise()`, and show how to use it with multiple functions. We'll then show a few uses with other verbs. We'll finish off with a bit of history, showing why we  prefer `across()` to our last approach (the `_if()`, `_at()` and `_all()` functions) and how you can translate your old code to use the new syntax.
...
* The second argument, `fns`, is a function or list of functions to apply to
  ...
  just get the underlying data; you'll see that technique used in
  `vignette("rowwise")`.)

Here are a few examples of `across()` in conjunction with its favourite verb, `summarise()`. But you can use `across()` with any dplyr verb, as you'll see a little later.
...
You can transform each variable with more than one function by supplying a named list in the second argument:
...
Control how the names are created with the `name` argument which takes a [glue](http://glue.tidyverse.org/) spec:
...
Prior versions of dplyr allowed you to apply a function to multiple columns in a different way: using functions with `_if`, `_at`, and `_all()` suffixes. These functions solved a pressing need and are used by many people, but are now superseded. That means that they'll stay around, but won't receive any new features and only critical bug fixes. 

### Why do we like `across()`?

Why did decide to move away from these functions in favour of `across()`?
...
1.  `across()` reduces the number of functions that dplyr needs to provide. 
    This makes dplyr easier to for you use (because there are fewer functions 
