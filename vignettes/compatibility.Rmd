...
Occasionally, you'll run into a situation where the `NAMESPACE` has changed and you need to conditionally import different functions. This typically occurs when functions are moved from one package to another. We try out best to provide automatic fallbacks, but this is not always possible. Often you can work around the problem by avoiding `importFrom` and using `utils::getFromNamespace()` instead. Do this where possible:

```{r, eval = FALSE}
if (utils::packageVersion("dplyr") > "0.5.0") {
  utils::getFromNamespace("build_sql", "dbplyr")(...)
} else {
  dplyr::build_sql(...)
}
```

Unfortunately, we can't specify `dbplyr::build_sql()` using `::` here because it will produce an `R CMD check` warning when run on a system that doesn't have a version of dbplyr with `build_sql()` exported from it.
