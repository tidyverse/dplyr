...
deprecated in version 0.7.0 and are now defunct.
...
defunct, we've turned the non-underscored verbs into S3 generics.

As an example, the `sf` package which defines objects 
of classes `c("sf", "data.frame")` implements `filter()`
by providing a method for the non-underscored generic:
