...
One core principle in dplyr is that you can refer to columns from the
tibble as if they were regular variables. However, the syntactic
uniformity of referring to bare column names hides semantical
...
A second difference has to do with the context of evaluation. Bare
names (also called symbols) are evaluated in the data context only.
Consequently, symbols must refer to data frame objects and not to
local variables. If you wish to refer to variables from the calling
context, you have to be explicit about it by unquoting (you can learn
more about unquoting in `vignette("programming")`):
...
By way of exception, selecting calls like `c(year, month, day)` or
`year:day` obey the same principle as symbols:

```{r}
pos <- 3
select(flights, year:(!! pos))
```

On the other hand, all other calls (and in particular calls to helper
functions like `starts_with()`) behave the opposite way. They cannot
refer to objects from the data but can see objects from the
surrounding context:
...
second argument, `year` is evaluated in the surrounding context
(because `identity()` is a function call) and represents the fifth
column.
