...
Empty groups can arise from two situations:
 - when one of the grouping variable in `group_by()` is a factor and one of its levels
   has no data
 - when all of the data from one group is `filter()`ed out. 
 
Previous versions of `dplyr` did not make empty groups, because: 
...
**this needs the branch feature-341-zero-length-groups** 
...
**this needs the branch feature-341-filter-preserve** 
...
*needs the `tidy-data-attributes-3489` branch*
...
Similarly, the indices themselves can be retrieved using the `rows()` generic: 
