...
* Recycled aggregates. These are not needed in R because vector recycling 
  means that any aggregate function can serve as a window function. 
  They are important in SQL, because the presence of an aggregation
  function usually tells the database to return only one row per group.
...
The ranking functions are all variations on a theme, only differing in how they handle ties:
...
This function uses a bit of non-standard evaluation, so I wouldn't recommend using it inside another function; instead you can use the simpler but less concise `with_order()`.
...
* The __partition clause__ specifies how the window function is broken down
  ...
  be partitioned into different groups, but database support varies, and it
  is not currently supported by dplyr.
...
* The __order clause__ controls the ordering (when it makes a difference).
  ...
  variable defines the order. In dplyr you do this with `arrange()`. Databases 
  vary in whether they fail if the order clause is missing when needed or 
...
if (has_lahman("postgres")) {
  ...
  print(translate_sql(mean(G), source = players_db, window = TRUE))
  print(translate_sql(cummean(G), source = players_db, window = TRUE))
  print(translate_sql(rank(G), source = players_db, window = TRUE))
  print(translate_sql(ntile(G, 2), source = players_db, window = TRUE))
  print(translate_sql(lag(G), source = players_db, window = TRUE))
}
```
...
if (has_lahman("postgres")) {
  ...
  print(translate_sql(cummean(G), source = players_by_year, window = TRUE))
  print(translate_sql(rank(), source = players_by_year, window = TRUE))
  print(translate_sql(lag(G), source = players_by_year, window = TRUE))
}
```
