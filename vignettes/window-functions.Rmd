...
departments <- group_by(empsalary, depname)

mutate(departments, mean(salary)) ->
  SELECT *, avg(salary) OVER (PARTITION BY depname)
  FROM empsalary

mutate(departments, cumsum(salary, in_order = age)) ->
  SELECT *, sum(salary) OVER (PARTITION BY depname ORDER BY age)
  FROM empsalary


summarise(departments, mean(salary)) ->
