...
`data_frame()` is a nice way to create data frames. It encapsulates best practices for data frames:
  ...
  * It evaluates its arguments lazily and in order:
  
    ```{r}
    data_frame(x = 1:5, y = x ^ 2)
    ```

  * It adds `tbl_df()` class to output so that if you accidentally print a large 
  ...
  * It changes the behaviour of `[` to always return the same type of object.
    Subsetting using `[` will always return a `tbl_df()` object; whereas
    subsetting using `[[` will always return a column.
    
    You should be aware of one subsetting case where a `tbl_df()` object  
