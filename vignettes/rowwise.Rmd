...
U`dplyr::summarise()` makes it really easy to summarise values across rows within one column. When combined with `rowwise()` it also makes it easy to summarise values across columns within one row. To see how, we'll start by making a little dataset:
...
`rowwise()` doesn't work only with functions that return a length-1 vector (akak summary functions); it can work with any function if the result in a list. This means that `rowwise()` and `mutate()` provide an elegant way to call a function many times with varying arguments, storing the outputs alongside the inputs. 
...
In more complicated problems, you might also want to vary the function being called. This tends to be a bit more of an awkward fit with this approach because because the columns in the input tibble will be less regular. But it's still possible, and it's a natural place to use `do.call()`:
...
The additional of `cur_data()`/`acrosS()` and the increased scope of `summarise()` means that `do()` is no longer needed, so it is now superseded.
