...
*   Without argument names: you could call functions that input and output 
    ...
    This has been superseded by `pick()` plus the more permissive `summarise()` 
    which can now create multiple columns and multiple rows.
  
    ```{r}
    mtcars %>% 
      group_by(cyl) %>% 
      summarise(head(pick(everything()), 1))
    ```
...
The addition of `pick()`/`across()` and the increased scope of `summarise()` means that `do()` is no longer needed, so it is now superseded.
