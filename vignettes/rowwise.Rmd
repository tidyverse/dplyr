...
**NB**: I use `df` (not `rf`) and `pick()` (not `c_across()`) here because `rowMeans()` and `rowSums()` take a multi-row data frame as input.
...
*   Without argument names: you could call functions that input and output 
    ...
    This has been superseded by `pick()` plus `morph()`, a variant of
    `summarise()` that can create multiple rows and columns per group.
  
    ```{r}
    mtcars %>% 
      group_by(cyl) %>% 
      morph(head(pick(everything()), 1))
    ```
...
The addition of `pick()`/`across()` and the increased scope of `summarise()`/`morph()` means that `do()` is no longer needed, so it is now superseded.
