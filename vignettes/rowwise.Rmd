...
But that returns the length of the column, not the length of the individual values. If you're an R documentation aficionado, you might know that there's already a base R function just for this purpose:
...
The life cycle of `rowwise()` was [questioning](https://lifecycle.r-lib.org/articles/stages.html#superseded-stages-1) for quite some time, partly because I didn't appreciate how many people needed the native ability to compute summaries across multiple variables for each row. As an alternative, we recommended performing row-wise operations with the `purrr::map()` functions. However, this was challenging because you needed to pick a map function based on the number of arguments that were varying and the type of result, which required quite a lot of knowledge of purrr functions.
