...
We can create new variables as functions of other ones. In base R we directly create a new variable using `$` and assign it a vector of values.
...
To rename a variable in base R, we first need to find which column is the one we want to replace, and then we need to override the name.
...
If we want every row in both data frames to be preserved in the merged data frame (denoted by base R as `all.x = T` and `all.y=T`), we use dplyr's `full_join()`. This doesn't drop any rows, even if they don't match. 
