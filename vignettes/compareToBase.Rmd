...
```{r}
# dplyr
mtcars %>% group_by(cyl) %>% mutate(avg_mpg_by_cyl = mean(mpg)) %>% ungroup()
```
...
## `relocate()`: Rename variables by name
...
iris[, setdiff(names(iris), c("Species", "Petal.Width"))] # all but two particular variables
```
