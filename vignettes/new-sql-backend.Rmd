...
Use `src_dbi()` to create a new S3 object with the correct structure. 

Next, implement a method for `src_desc()` that briefly describes the connection:

```{r}
#' @export
src_desc.PostgreSQLConnection <- function(con) {
  info <- dbGetInfo(con)
...
Before continuing, check that you can create a connection to a local database, and that you get a listing of the existing tables. If you have a problem at this point, you may need to check the DBI backend.  Next make sure you can connect to an existing table, and that the results are printed reasonably. If not, that may indicate your database uses a non-standard DBI interface, and you'll need to fix that before continuing.
