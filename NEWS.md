...
* `summarise_each()` and `mutate_each()` are deprecated in favour of a
  more complete family of colwise verbs (see below).
...
* `summarise_each()` and `mutate_each()` are replaced with
  `summarise_all()` and `mutate_all()`. Column specification are now
  handled with `summarise_at()` and `mutate_at()`, which take either a
  character vector of columns names, a numeric vector of column
  positions, or a column specification with `select()` semantics
  generated by the `columns()` helper. In addition, `summarise_if()`
  and `mutate_if()` accept a predicate function or a logical vector
  (currently only compatible with local sources). All these functions
  can now take ordinary functions instead of a list of functions
  generated by `funs()` (though this is only useful for local
  sources). (#1845, @lionel-)
