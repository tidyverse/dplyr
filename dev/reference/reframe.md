...
input, `reframe()` is a more general workhorse with no requirements on
the number of rows returned per group.
...
[`summarise()`](https://dplyr.tidyverse.org/dev/reference/summarise.md)
much more often than `reframe()`, but `reframe()` can be particularly
helpful when you need to apply a complex function that doesn't return a
