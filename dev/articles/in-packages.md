...
along 4 helpers for performing various types of joins:
...
You’ll notice that
[`dplyr::closest()`](https://dplyr.tidyverse.org/dev/reference/join_by.md)
isn’t an exported function from dplyr
([`dplyr::between()`](https://dplyr.tidyverse.org/dev/reference/between.md)
and [`base::within()`](https://rdrr.io/r/base/with.html) do happen to be
preexisting functions). If you use
[`closest()`](https://dplyr.tidyverse.org/dev/reference/join_by.md) in
your package, then this will cause an `R CMD check` note letting you
know that you’ve used a symbol that doesn’t belong to any package.
...
[`vignette("programming", package = "dplyr")`](https://dplyr.tidyverse.org/dev/articles/programming.md).
...
even though that branch will never run. You can get around this by using
...
[`reframe()`](https://dplyr.tidyverse.org/dev/reference/reframe.md) as
long as you also require `dplyr (>= 1.1.0)` in your `DESCRIPTION` file.
...
[`summarise()`](https://dplyr.tidyverse.org/dev/reference/summarise.md)
in combination with
[`across()`](https://dplyr.tidyverse.org/dev/reference/across.md), which
was introduced in dplyr 1.0.0.
