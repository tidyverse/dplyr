#' @section Debugging:
#'
#' To see exactly what SQL is being sent to the database, you use
#' \code{\link{show_query}()} and \code{\link{explain}()}.
#'
#' @section Grouping:
#'
#' Typically you will create a grouped data table is to call the \code{\link{group_by}()}
...
#'
#' Note that \code{\link{do}()} is not lazy since it must pull the data into R.
