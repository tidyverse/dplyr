...
void install_count_handlers(HybridHandlerMap& handlers) {
  ...
  handlers[ Rf_install("n") ] = HybridHandler(count_prototype, ns_dplyr, ns_dplyr["n"]) ;
  handlers[ Rf_install("n_distinct") ] = HybridHandler(count_distinct_prototype, ns_dplyr, ns_dplyr["n_distinct"]) ;
}
