...
test_that("filter results independent of data tbl (simple)", {
  ...
  compare_tbls(tbls, function(x) x %>% filter(a > 6), ref = expected, convert = TRUE)
})
...
test_that("filter captures local variables", {
  ...
  compare_tbls(tbls, function(x) x %>% filter(b %in% sel), ref = expected, convert = TRUE)
})
