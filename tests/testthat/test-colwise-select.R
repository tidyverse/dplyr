...
test_that("colwise select() / rename() give meaningful errors", {
  ...
  # colwise rename()
  expect_snapshot_error(df %>% rename_all())
  expect_snapshot_error(df %>% rename_if(is_integerish))
  expect_snapshot_error(df %>% rename_at(vars(x:y)))
  expect_snapshot_error(df %>% rename_all(list(tolower, toupper)))

  # colwise select()
  expect_snapshot_error(df %>% select_all(list(tolower, toupper)))
  expect_snapshot_error(df %>% select_if(function(.x) 1))
  expect_snapshot_error(df %>% select_if(function(.x) c(TRUE, TRUE)))
})
