...
check_hybrid_result <- function(expr, ..., expected, test_eval = TRUE) {
  check_hybrid_result_(tidy_capture(expr), ..., expected = expected, test_eval = test_eval)
}
...
check_not_hybrid_result <- function(expr, ..., expected, test_eval = TRUE) {
  check_not_hybrid_result_(tidy_capture(expr), ..., expected = expected, test_eval = test_eval)
}
...
expect_hybrid_error <- function(expr, ..., error) {
  expect_hybrid_error_(tidy_capture(expr), ..., error = error)
}
...
expect_not_hybrid_error <- function(expr, ..., error) {
  expect_not_hybrid_error_(tidy_capture(expr), ..., error = error)
}
