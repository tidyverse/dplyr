...
class process_data<CLASS, NaturalDataFrame> {
public:
  process_data(const NaturalDataFrame& gdf, CLASS* chunk_source_) : chunk_source(chunk_source_), n(gdf.nrows()) {}

  SEXP run() {
    return chunk_source->process_chunk(NaturalSlicingIndex(n)) ;
  }
