...
- [`across()`](https://dplyr.tidyverse.org/reference/across.md),
  ...
  [`pull()`](https://dplyr.tidyverse.org/reference/pull.md) use **tidy
  selection** so you can easily choose variables based on their
...
[`?dplyr_tidy_select`](https://dplyr.tidyverse.org/reference/dplyr_tidy_select.md).
...
[`group_by()`](https://dplyr.tidyverse.org/reference/group_by.md) or a
...
[`mutate()`](https://dplyr.tidyverse.org/reference/mutate.md) which
allow you to add multiple columns by returning a data frame:
...
tells [`across()`](https://dplyr.tidyverse.org/reference/across.md) to
*unpack* the data frame returned by `quantile_df()` into its respective
columns, combining the column names of the original columns (`x` and
`y`) with the column names returned from the function (`val` and
`quant`).
...
restricted to returning 1 row summaries per group, but
[`reframe()`](https://dplyr.tidyverse.org/reference/reframe.md) lifts
this restriction:
...
[`across()`](https://dplyr.tidyverse.org/reference/across.md) to control
...
1.  dplyr’s
    [`filter()`](https://dplyr.tidyverse.org/reference/filter.md) is
