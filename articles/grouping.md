...
[`group_by()`](https://dplyr.tidyverse.org/reference/group_by.md): it
...
Or use [`tally()`](https://dplyr.tidyverse.org/reference/count.md) to
count the number of rows in each group. The `sort` argument is useful if
you want to see the largest groups up front.
...
[`mutate()`](https://dplyr.tidyverse.org/reference/mutate.md) **before**
the [`group_by()`](https://dplyr.tidyverse.org/reference/group_by.md):
...
Use
[`group_vars()`](https://dplyr.tidyverse.org/reference/group_data.md) if
you just want the names of the grouping variables:
...
If you apply
[`group_by()`](https://dplyr.tidyverse.org/reference/group_by.md) to an
already grouped dataset, will overwrite the existing grouping variables.
For example, the following code groups by `homeworld` instead of
`species`:
...
[`relocate()`](https://dplyr.tidyverse.org/reference/relocate.md) behave
identically with grouped and ungrouped data because they only affect the
name or position of existing columns. Grouped
[`select()`](https://dplyr.tidyverse.org/reference/select.md) is almost
...
[`ungroup()`](https://dplyr.tidyverse.org/reference/group_by.md). (This
design is possibly a mistake, but we’re stuck with it for now.)

### `arrange()`

Grouped [`arrange()`](https://dplyr.tidyverse.org/reference/arrange.md)
is the same as ungrouped
[`arrange()`](https://dplyr.tidyverse.org/reference/arrange.md), unless
you set `.by_group = TRUE`, in which case it will order first by the
grouping variables.
...
[`mutate()`](https://dplyr.tidyverse.org/reference/mutate.md) give the
same results. They differ when used with summary functions:
...
A grouped [`filter()`](https://dplyr.tidyverse.org/reference/filter.md)
effectively does a
...
[`slice()`](https://dplyr.tidyverse.org/reference/slice.md) and friends
...
rows within a group. For example, we can select the first observation
within each species:
...
[`slice_min()`](https://dplyr.tidyverse.org/reference/slice.md) to
