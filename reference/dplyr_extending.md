...
- [`arrange()`](https://dplyr.tidyverse.org/reference/arrange.md),
  ...
  [`slice()`](https://dplyr.tidyverse.org/reference/slice.md) (and the
  rest of the `slice_*()` family),
...
- [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.md)
  ...
  [`relocate()`](https://dplyr.tidyverse.org/reference/relocate.md) if
  either `.before` or `.after` are supplied.

- [`summarise()`](https://dplyr.tidyverse.org/reference/summarise.md)
  and [`reframe()`](https://dplyr.tidyverse.org/reference/reframe.md)
  work similarly to
  [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.md) but the
  data modified by `dplyr_col_modify()` comes from
...
- [`select()`](https://dplyr.tidyverse.org/reference/select.md) uses 1d
  `[` to select columns, then `names<-` to rename them.
  [`rename()`](https://dplyr.tidyverse.org/reference/rename.md) just
  uses `names<-`.
  [`relocate()`](https://dplyr.tidyverse.org/reference/relocate.md) just
  uses 1d `[`.
...
- [`distinct()`](https://dplyr.tidyverse.org/reference/distinct.md) does
  a [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.md) if any
  expressions are present, then uses 1d `[` to select variables to keep,
  then `dplyr_row_slice()` to select distinct rows.

Note that
[`group_by()`](https://dplyr.tidyverse.org/reference/group_by.md) and
[`ungroup()`](https://dplyr.tidyverse.org/reference/group_by.md) don't
