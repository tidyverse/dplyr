...
- ...:
  ...
  [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.md) step
  before the `group_by()`. Computations are not allowed in
  [`nest_by()`](https://dplyr.tidyverse.org/reference/nest_by.md). In
  `ungroup()`, variables to remove from the grouping.
...
- x:
  ...
  A [`tbl()`](https://dplyr.tidyverse.org/reference/tbl.md)
...
call to [`arrange()`](https://dplyr.tidyverse.org/reference/arrange.md)
and set the `.locale` argument. For example:
